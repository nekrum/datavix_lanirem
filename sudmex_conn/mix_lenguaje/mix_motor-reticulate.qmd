...
[**Jupyter** es un componente de JupyterNotebooks que permite ejecutar codigo de R y python]{.aside}

Si usamos la opción `#| python.reticulate = FALSE` se puede ejecutar el codigo pero las variables generadas en un lenguaje
...
[**Reticulate** es un programa que permite la ejecución de codigo Python dentro de R]{.aside} que puede ser instalado 
...
Podemos usar la variable generada llamando el objeto `r` y la variable que generamos en el codigo anterior es un atributo
de ese objeto.

```{python}
# Usando variable generada en el codigo R
y = len(r.x)
y
```
...
```{r}
# Usando variable creada en el chunck de Pyuthon
py$y
```
...
Quarto también puede ejecutar codigo R y Python usando unicamente el motor de Jupyter instalando un kernel para R
